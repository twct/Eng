    ...
    m_context(context)
{
    ...
    m_sprite = &m_context->registry.emplace<Sprite>(entity);
    ...
    m_transform = &m_context->registry.emplace<Vector2f>(entity, 15.f, 15.f);
    m_context->registry.emplace<Collider>(entity, 32, 32);
    // m_sprite.texture(m_context->assetLoader->getTexture(atlasTexture.id));
    // m_sprite.atlas(atlas);
    // m_sprite.playAnimation(atlas.defaultAnimation);
